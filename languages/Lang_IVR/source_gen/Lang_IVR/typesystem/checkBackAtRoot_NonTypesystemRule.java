...
public class checkBackAtRoot_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode back, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if (SNodeOperations.isInstanceOf(SNodeOperations.getParent(back), CONCEPTS.IncludeActivities$o_)) {
      if (SNodeOperations.isInstanceOf(SNodeOperations.getParent(SNodeOperations.cast(SNodeOperations.getParent(back), CONCEPTS.IncludeActivities$o_)), CONCEPTS.MenuList$_b)) {
        if (SNodeOperations.isInstanceOf(SNodeOperations.getParent(SNodeOperations.cast(SNodeOperations.getParent(SNodeOperations.cast(SNodeOperations.getParent(back), CONCEPTS.IncludeActivities$o_)), CONCEPTS.MenuList$_b)), CONCEPTS.DialPadTemplate$lF)) {
          {
            final MessageTarget errorTarget = new NodeMessageTarget();
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(back, "Back at root node", "r:883ed43f-b821-4541-878e-1c26ac000d73(Lang_IVR.typesystem)", "5461963470562044700", null, errorTarget);
            {
